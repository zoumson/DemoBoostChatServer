...
static unsigned short serverRoomNum = 5;
static unsigned short serverRoomNumOffset = 100;
static unsigned short roomMaxParticipant = 3;

boost::mutex endSessionMutex;
std::thread endSessionThread;
std::thread serverThread;
//static boost::asio::io_context serverBoostChannelOS;
//static boost::asio::high_resolution_timer timer(serverBoostChannelOS);
...
public:
    ...
    void welcome(chat_participant_ptr participant)
    {
        ...
        buffer[cnt::ZA_PACKET::BODY_1] = 'w';
        buffer[cnt::ZA_PACKET::BODY_2] = 'e';
        buffer[cnt::ZA_PACKET::BODY_3] = 'l';
        buffer[cnt::ZA_PACKET::BODY_4] = 'c';
        buffer[cnt::ZA_PACKET::BODY_5] = 'o';
                ...
                io_context_.dispatch([&]() 
                    {
                        ...
                        timer.wait();
                        boost::lock_guard<boost::mutex> loc(endSessionMutex);
    ...
    void do_write()
    {
        ...
        boost::asio::async_write(socket_,
            ...
            [this, self](boost::system::error_code ec, std::size_t /*length*/)
            {
                if (!ec)
                {
                    std::cout << "Server sent a message to a client" << std::endl;
...
public:
    ...
    {
        std::cout << "Server at reception desk" << std::endl;
        do_room();
        do_accept();
    }
    ...
    void do_accept()
    {
        ...
        acceptor_.async_accept(
            [this](boost::system::error_code ec, boost::asio::ip::tcp::socket socket)
            {
                if (!ec)
                {
                    std::cout << "Client[" << socket.remote_endpoint() << "] at reception desk[" << socket.local_endpoint() << "][ACCEPTED]" << std::endl;
                    std::make_shared<chat_session>(io_context_, std::move(socket), room_[client_num_ % room_num_])->start(std::chrono::seconds(30));
                }
                else
                {
                }
...
int main(int argc, char* argv[])
{
    try
    {
        //testJson();
        //testFFMPEG();
        boost::asio::io_context serverBoostChannelOS;

        std::list<chat_server> servers;
        boost::asio::ip::tcp::endpoint serverBoostAddress(boost::asio::ip::tcp::v4(), serverPortNum);
        servers.emplace_back(serverBoostChannelOS, serverBoostAddress, serverRoomNum, serverRoomNumOffset);
        serverThread = std::thread([&]() { serverBoostChannelOS.run(); });
